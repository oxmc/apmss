...
if [ -f /usr/bin/yad ];then
  echo -e "$package_list" | yad --list --no-click --no-columns --separator='\n' \
	--title='ApachePhpMariadbSqlServer' --center --window-icon="${DIRECTORY}/icons/logo-64.png" \
  ...
  echo -e "$package_list" | zenity --title='ApachePhpMariadbSqlServer' --window-icon="${DIRECTORY}/icons/logo-64.png" \
...
x-terminal-emulator --title="Installing progress" -e "sudo apt install -y $PKG_LIST;echo -e '\nInstallation finished.\nClosing in 5 seconds.';sleep 5"
